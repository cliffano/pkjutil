...
const DEPENDENCY_TYPES = ['dependencies', 'devDependencies', 'peerDependencies', 'optDependencies'];
...
PkjUtil.prototype.traverseDependencies = function (cb) {
  if (fs.existsSync('package.json') || fs.existsSync('node_modules')) {
    ...
    cb(new Error('Either package.json file or node_modules directory must exist'));
  }
};
...
/**
 * Set the node engine version.
 *
 * @param {String} node engine version, e.g. >= 4.0
 * @param {Function} cb: standard cb(err, result) callback
 */
PkjUtil.prototype.setNodeEngine = function (version, cb) {
